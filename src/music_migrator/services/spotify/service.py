...
def _track_from_spotify(raw: dict[str, Any] | None) -> Track | None:
    ...
    return Track(source_id, title, artists, album.get("name"), duration_ms / 1000 if duration_ms is not None else None, external_ids.get("isrc"))
...
class SpotifySource:
    ...
    @classmethod
    def authenticate(cls, config: SpotifyConfig, session_path: Path = Path(".spotify-session.json")) -> "SpotifySource":
        return cls(create_spotify_client(config, session_path))

    def playlists(self) -> Iterator[Playlist]:
        user_id = self._current_user_id()
        for raw in _pages(lambda offset: self._client.current_user_playlists(limit=50, offset=offset)):
            ...
            if playlist_id and name:
                yield Playlist(playlist_id, name, raw.get("description") or "")
    ...
    def playlist(self, playlist_id: str) -> Playlist:
        ...
        return Playlist(raw["id"], raw["name"], raw.get("description") or "")

    def playlist_tracks(self, playlist_id: str) -> Iterator[Track]:
        pages = _pages(lambda offset: self._client.playlist_items(playlist_id, limit=50, offset=offset, additional_types=("track",)))
        yield from self._tracks_from_pages(pages)

    def saved_tracks(self) -> Iterator[Track]:
        yield from self._tracks_from_pages(_pages(lambda offset: self._client.current_user_saved_tracks(limit=50, offset=offset)))

    def _current_user_id(self) -> str:
        if self._user_id is None:
            user_id = self._client.current_user().get("id")
            if not user_id:
    ...
    def _tracks_from_pages(cls, entries: Iterator[dict[str, Any]]) -> Iterator[Track]:
        for entry in entries:
            track = _track_from_spotify(entry.get("item") or entry.get("track"))
...
class SpotifyDestination:
    ...
    @classmethod
    def authenticate(cls, config: SpotifyConfig, session_path: Path = Path(".spotify-session.json")) -> "SpotifyDestination":
    ...
    def playlists_by_name(self) -> dict[str, dict[str, Any]]:
        ...
        user_id = self._current_user_id()
        for playlist in _pages(lambda offset: self._client.current_user_playlists(limit=50, offset=offset)):
            owner_id = (playlist.get("owner") or {}).get("id")
            if owner_id != user_id and not playlist.get("collaborative", False):
                continue
            name, playlist_id = playlist.get("name"), playlist.get("id")
        ...
        if duplicates:
            raise ValueError(f"Spotify contains duplicate playlist names: {', '.join(sorted(duplicates))}")
        return result

    def create_playlist(self, name: str, description: str) -> dict[str, Any]:
        return self._client.current_user_playlist_create(name, public=False, description=description)

    def search_tracks(self, source: Track, limit: int = 20, before_request: Callable[[], None] | None = None) -> list[Track]:
        candidates: dict[str, Track] = {}
        queries = ([f"isrc:{source.isrc}"] if source.isrc else []) + [f"{source.title} {source.artists[0]}"]
        for query in queries:
            if before_request:
                before_request()
            results = self._client.search(q=query, type="track", limit=max(1, min(limit, SPOTIFY_SEARCH_LIMIT)))
            for raw in (results.get("tracks") or {}).get("items") or []:
                candidate = _track_from_spotify(raw)
                if candidate:
                    candidates[candidate.source_id] = candidate
            if source.isrc and any(item.isrc and item.isrc.casefold() == source.isrc.casefold() for item in candidates.values()):
                break
    ...
    def playlist_track_ids(self, playlist: Any) -> list[str]:
        ...
        ids = []
        for entry in _pages(lambda offset: self._client.playlist_items(playlist_id, limit=SPOTIFY_PLAYLIST_PAGE_SIZE, offset=offset, additional_types=("track",))):
            raw = entry.get("item") or entry.get("track")
            if raw and raw.get("id"):
                ids.append(raw["id"])
        return ids

    def append_playlist_tracks(self, playlist: Any, track_ids: list[str], *, expected_before: list[str]) -> None:
        playlist_id = self._playlist_id(playlist)
        desired = [*expected_before, *track_ids]
        self._add_tracks(playlist_id, desired, start=len(expected_before))

    def replace_playlist_tracks(self, playlist: Any, track_ids: list[str], *, original_track_ids: list[str]) -> None:
        playlist_id = self._playlist_id(playlist)
        try:
    ...
    def saved_track_ids(self) -> set[str]:
        ids: set[str] = set()
        for entry in _pages(lambda offset: self._client.current_user_saved_tracks(limit=50, offset=offset)):
            raw = entry.get("item") or entry.get("track")
            if raw and raw.get("id"):
                ids.add(raw["id"])
        return ids
    ...
    def add_saved_tracks(self, track_ids: list[str]) -> int:
        ...
        for start in range(0, len(track_ids), SPOTIFY_LIBRARY_BATCH_SIZE):
            ...
            missing = [track_id for track_id, present in zip(chunk, existing, strict=True) if not present]
            if missing:
